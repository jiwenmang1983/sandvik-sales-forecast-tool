...
- 实现销售预测的在线填报、审批和汇总
- 支持多层级组织架构（销售 → 直线经理 → 大区负责人 → 最终审批人（销售总监））
...
- **当前周期确定规则：** 同一时间点只有一个周期在有效填报期内（FillTimeStart ≤ now ≤ FillTimeEnd 或 Extension窗口）。公司层面强制周期时间不重叠，系统按当前时间自动取落在窗口内的周期

### 2.2 审批状态机

空白 → 草稿（已保存）→ 提交至直线经理审批 → 提交至区域总监审批 → 提交至销售总监审批 → 审批已完成。

- 草稿提交后原记录状态直接变更，**不新建记录**
- 直线经理审批后推进到上一层；区域总监审批后推进到 销售总监 最终审批后流程结束
...
| 字段   | 规则                                                      |
| ---- | ------------------------------------------------------- |
| 客户   | 下拉选择，**受品牌过滤**（Customer.Brand = 销售人员person.Brand）      |
...
**数据状态：** 草稿 / 审批中：已提交直线经理 / 审批中：已提交区域总监/ 审批中：已提交销售总监/ 已通过 
...
| 动作  | 行为                                           |
| --- | -------------------------------------------- |
| 通过  | 自动推进到上一层审批节点；若已是最高节点则流程结束（status=已通过）        |
...
- 销售只能填报自己品牌下的客户（`销售人员person.Brand = Customer.Brand`）
- **品牌过滤在后端查询层强制执行，不可绕过**
...
| 姓名        | ✅   |                                                   |
| 邮箱        | ✅   | M365账号，登录唯一标识                                     |
| 角色        | ✅   | 下拉选择：SYS_ADMIN / 销售总监 / 大区负责人 / 直线经理 / 销售人员 |
| 关联OrgNode | ✅   | 下拉选择                                              |
| 品牌        | ❌   | 同步组织架构中的品牌                                          |
| 状态        | ✅   | 启用/停用                                             |
...
| -------------- | -------------------- |
| 销售（销售人员）      | 仅自己填报的数据             |
| 直线经理（直线经理）  | 自己 + 所管辖所有销售的数据      |
| 区域总监（大区负责人） | 自己 + 手下所有销售的数据（递归向上） |
| 销售总监（Frank Tao） | 全量数据                 |
...
山工多品牌，每个销售只归属一个品牌，每个客户也只归属一个品牌。过滤逻辑：`销售人员person.Brand = Customer.Brand`，确保销售只能填报自己品牌下的客户。**品牌过滤在后端查询层强制执行，不可绕过。**
...
预测填报分为**页面一（周期列表页）**和**页面二（填报工作台）**两层。
...
│ 预测填报                                                    │
├─────────────────────────────────────────────────────────────┤
│ 筛选栏：[状态 ▼] [FC Name关键字 🔍] [重置]               │
├─────────────────────────────────────────────────────────────┤
│ 表格                                                        │
│ FC Name | 填报时间 | 销售预测周期 | 状态 | 审批状态 | 操作 │
└─────────────────────────────────────────────────────────────┘
```
...
| 列 | 说明 |
| --- | --- |
| FC Name | 周期名称，如"2026FC1" |
| 填报时间 | FillTimeStart ~ FillTimeEnd |
| 销售预测周期 | 预测起止年月，如"2026-07 ~ 2027-03" |
| 状态 | 开放中 / 已关闭 |
| 审批状态 | 待审批 / 已审批 / 已驳回 |
| 操作 | [填报] [历史] |

**操作按钮：**
- **[填报]** → 进入该周期的填报工作台（页面二）；若当前不在填报窗口内则按钮禁用并提示
- **[历史]** → 查看该周期的历史填报记录
...
**路由：** `/forecast/workbench`（从周期列表点击填报进入，URL 可带 periodId 参数）

**职责：** 销售在选定周期内，按"客户+产品+月份"填写订单/开票数量和金额。

**工作台工具栏左侧（4个信息标签）：**

```
角色：销售  |  流程：标准流程  |  周期：2026FC1  |  填报窗口：2026-04-01 ~ 2026-04-30
```
...
| --- | --- |
| 下载模板 | 下载空白 Excel 模板 |
| 下载数据 | 导出当前筛选条件下的填报数据为 Excel |
| 导入 | 上传填写好的 Excel，批量写入/覆盖数据 |
| 复制上期 | 从历史周期复制同客户+同产品数据 |
| 新增行 | 在表格内新增一行空白预测记录 |
| 保存草稿 | 保存当前填报内容，不提交 |
| 提交审批 | 提交流程，进入审批状态 |
...
| 筛选项 | 说明 |
| --- | --- |
| 客户 | 下拉，**受品牌过滤**（Customer.Brand = Salesperson.Brand） |
| 开票公司 | 下拉，无品牌限制 |
| 业绩归属 | 下拉，来自 OrgNode |
| 销售大区 | 下拉，来自 OrgNode |
| PA | 下拉，产品L1 |
...
| 订单数量 | 整数，可为0，不可为负 |
| 订单金额 | 整数（元），可为0，不可为负 |
| 订单单价 | **自动计算**（订单金额÷订单数量），只读 |
| 开票数量 | 整数，可为0，不可为负 |
| 开票金额 | 整数（元），可为0，不可为负 |
| 开票单价 | **自动计算**（开票金额÷开票数量），只读 |

**表头分组（每组月度列）：**

```
┌──────────┬──────────┬──────────┬─────┬──────────┬──────────┬──────────┐
│ 客户     │ 开票公司  │ 业绩归属  │ ... │ 订单数量  │ 订单金额  │ 订单单价  │ ...
├──────────┼──────────┼──────────┼─────┼──────────┼──────────┼──────────┤
│          │          │          │     │ 开票数量  │ 开票金额  │ 开票单价  │ ...
└──────────┴──────────┴──────────┴─────┴──────────┴──────────┴──────────┘
   （固定列）                              （每月重复6列，按月份循环）
...
**合计行：** 表格底部有一行"合计"，自动 sum 各数值列。

**分页信息栏：** 底部显示分页信息（默认每页20条）。

**新增行逻辑（[新增行]按钮）：**
- 点击 → 在表格中新增一行空白记录行
- 客户/开票公司/产品线等字段为空，需手动选择
- 月度列初始值均为空/0
...
**复制上期逻辑：**
1. 点击 → 弹出窗口「选择来源周期」
2. 选择历史周期 → 系统填入该周期中同客户+同产品+同月份数据
3. 复制后为草稿状态

**保存草稿：** 仅保存当前填报内容，不提交。

**提交审批：** 提交后整张工作台的数据进入审批流，提交人不可自行撤回。
...
| E | Sub PA-2 | 可空 |
| F | Sub PA-3 | 可空 |
| G | Sub PA-4 (产品名称) | 可空，模糊匹配 |
| H~M | 订单数量（按月份） | 整数 |
| N~S | 订单金额（按月份） | 整数（元） |
| T~Z | 开票数量（按月份） | 整数 |
| AA~AF | 开票金额（按月份） | 整数（元） |

**批量导入行为：** 相同（销售+周期+客户+产品+月份）覆盖更新。
...
| 姓名        |                                              |
| 邮箱        | 即登录账号                                        |
| 角色        | SYS_ADMIN / 销售总监 / 大区负责人 / 直线经理 / 销售人员 |
| 关联OrgNode |                                              |
| 品牌        |                                              |
| 状态        | 启用/停用                                        |
| 操作        | [编辑] [停用/启用]                                 |
...
org_nodes (id, name, parent_id, role, region, brand,   -- brand: 销售归属的品牌
           company,           -- 所属公司
           销售人员_region,      -- 业绩归属区域（自动填入填报页面）
           销售人员_district,    -- 销售大区（自动填入填报页面）

...
**3.7.6 已知限制与 workaround**

1. **`--resume` session ID 不稳定**：MiniMax server 会在会话结束后清除 session，跨进程 `--resume` 通常失败。**tmux 会话本身是上下文保持的核心机制**，不依赖 `--resume`。
2. **MiniMax API 429 限制**：4500单位/5小时窗口。Phase 1 开发期间注意用量，大任务前用 `curl` 先测 API 状态。
